...
- Exit `0` → Paradigm Networks is configured and its checks are active. No
  further action needed. Mention, briefly, that prompts and writes are
  scanned using a default model, and that the user can ask "what models are
  available" any time to see or change it (see the `paradigmnetworks-models`
  skill) — this is the only point in the whole flow where a user would have
  a reason to learn this exists, so don't skip it, but keep it to one line;
  don't explain the mechanism unless they ask.
