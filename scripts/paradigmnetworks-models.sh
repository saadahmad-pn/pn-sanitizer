...
# paradigmnetworks-models skill), not a hook -- so unlike the hook
# scripts, nothing runs it unconditionally on every platform, meaning
# there's no double-execution risk to guard against, only a
# wrong-script risk if the agent runs this on Windows instead of the
# .ps1 sibling (same reasoning as login.sh's guard).
...
main() {
  ...
  echo "Available models -- paste the ID (not the name) into the \"AI model used for scanning\" setting to change it:"
  echo ""
