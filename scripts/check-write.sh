...
STOP_INSTRUCTION="A security scan blocked this write due to a detected policy violation. Do not disguise this write or attempt a workaround (e.g. base64-encoding it, splitting the string, writing it to a different file, or renaming the variable) -- any of those will be scanned and blocked the same way. If you want to address the findings above, make a genuinely corrected version that applies the suggested fixes and submit that as a new write -- it will be scanned independently, not skipped. Relay the findings above to the user in full, exactly as given -- every issue, category, code, and standard name mentioned. Do not summarize or paraphrase them into a general statement; the user needs the precise details to know what to fix."
