...
if [[ $TOTAL_FAILED -eq 0 ]]; then
  ...
  echo ""
  echo "Test Coverage:"
  echo "  • Unit Tests:           51 assertions (lib/common.sh, lib/git-utils.sh, pn_config.sh)"
  echo "  • Integration Tests:    27 tests (check-session.sh, check-prompt.sh, check-write.sh, check-repo-context.sh)"
  echo "  • Error Scenarios:      9 tests (edge cases, malformed input, file system errors)"
  echo ""
  echo "Dependencies:"
