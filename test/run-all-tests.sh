...
if [[ $TOTAL_FAILED -eq 0 ]]; then
  ...
  echo ""
  echo "Test Coverage:"
  echo "  • Unit Tests:           48 assertions (lib/common.sh, lib/multipart.sh, pn_config.sh)"
  echo "  • Integration Tests:    24 tests (check-session.sh, check-prompt.sh, check-write.sh, check-response.sh)"
